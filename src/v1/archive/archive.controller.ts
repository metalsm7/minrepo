import { Controller, Get, Post, Put, Param, Req, Res, HttpStatus, StreamableFile } from '@nestjs/common';
...
export class ArchiveController {
    ...
    @Put('*/:version')
    async replace(@Param('version') version: string, @Req() req: Request, @Res() res: Response) {
        //
        const archiveInfo: ArchiveInfo = {
          group_id: req.path.replace(RegExps.ReqArchive, '$1').replace(/\//g, '.'),
          artifact_id: req.path.replace(RegExps.ReqArchive, '$2'),
        ...
        if (repo === null) {
            ...
            //
            archiveInfo.repo_id = repo.repo_id;
            rtn_val = await this.archiveService.replaceRepo(archiveInfo, repo_detail.repo_detail_id, ver_req > ver_current);
        }
